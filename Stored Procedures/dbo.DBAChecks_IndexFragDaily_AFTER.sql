...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[DBAChecks_IndexFragDaily_AFTER](@HTML varchar(max) out)
AS
/* 
...
IF ''?'' <> ''master'' AND ''?'' <> ''model'' AND ''?'' <> ''msdb'' AND ''?'' <> ''tempdb'' AND ''?'' <> ''Clinical''
...
DROP TABLE #dbindex
GO
GRANT ALTER ON  [dbo].[DBAChecks_IndexFragDaily_AFTER] TO [db_sp_alter]
GO
GRANT EXECUTE ON  [dbo].[DBAChecks_IndexFragDaily_AFTER] TO [db_sp_executor]
GO
GRANT VIEW DEFINITION ON  [dbo].[DBAChecks_IndexFragDaily_AFTER] TO [db_sp_viewer]
GO
