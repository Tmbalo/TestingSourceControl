...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[DBAChecks_ErrorLogDaily](@NumDays int, @HTML varchar(max) out)
AS
/* 
...
AND ErrorMessage NOT LIKE '%AppDomain%'
AND ErrorMessage NOT LIKE '%PGWC\VM-BIOL-SQL03$%'
AND ErrorMessage NOT LIKE '%DbMgrPartnerCommitPolicy%'
AND ErrorMessage NOT LIKE '%successfully%'
AND ErrorSource NOT IN ('Logon')
AND ErrorMessage NOT LIKE '%Configuration option%'
...
GO
GRANT ALTER ON  [dbo].[DBAChecks_ErrorLogDaily] TO [db_sp_alter]
GO
GRANT EXECUTE ON  [dbo].[DBAChecks_ErrorLogDaily] TO [db_sp_executor]
GO
GRANT VIEW DEFINITION ON  [dbo].[DBAChecks_ErrorLogDaily] TO [db_sp_viewer]
GO
