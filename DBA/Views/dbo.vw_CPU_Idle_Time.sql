SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO

CREATE VIEW [dbo].[vw_CPU_Idle_Time]
as
SELECT  
    CounterName, 
    InstanceName, 
  --try_cast(  TRY_CAST(FORMAT(r(CounterValue) / 1048576.0, 2,1), 'n2') AS decimal(5,2)) as int) AS 'ValueMB' , 
	cast(round(COUNTERVALUE, 2,1) as int) AS 'Value'  ,
   CAST(LEFT(CounterDateTime, CHARINDEX('.', CounterDateTime) -1) AS datetime) AS Date
--into cpu_test
FROM dbo.CounterDetails CDetails
    INNER JOIN dbo.CounterData CData ON CData.CounterID = CDetails.CounterID
    INNER JOIN DisplayToID DToID ON DToID.GUID = CData.GUID
WHERE ObjectName = 'Processor' AND 
    CDetails.CounterName in ('% Processor Time') AND CounterValue <> 0
--WHERE CDetails.CounterName = 'Disk Read IO/sec'
--ORDER BY CounterDateTime;
GO
