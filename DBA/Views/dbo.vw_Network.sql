...
SET ANSI_NULLS ON
GO
CREATE VIEW [dbo].[vw_Network]
as
SELECT  
    CounterName, 
    InstanceName, 
  try_cast(  TRY_CAST(FORMAT(round((CounterValue) / 1048576.0, 2,1), 'n2') AS decimal(5,2)) as int) AS 'Value MB' , 
	--COUNTERVALUE  ,
...
WHERE ObjectName = 'Network Interface' AND 
    CDetails.CounterName = 'Bytes Total/sec' AND CounterValue <> 0
